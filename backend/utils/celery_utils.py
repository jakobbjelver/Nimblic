...
from data_processor.Statistics.object_analysis import object_analysis
...
def create_celery_tasks(celery):
    ...
    def async_detect_change_points(self, dataframe_json):
        logger.info('Task started: async_detect_change_points')
    ...
    def async_object_analysis(self, dataframe_json):
        ...
        try:
            print("Analyzing objects asynchronously...")

            dataframe = pd.read_json(dataframe_json, orient='records')

            analysis_data = object_analysis(dataframe)
