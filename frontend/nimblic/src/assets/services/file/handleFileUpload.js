 ...
 */
export function handleFileUpload(file, onUploadStart, onUploadEnd, setUploadData, setIsUploading, setNotifications) {
    return new Promise((resolve, reject) => {
        ...
        if (!validateFileType(file.type)) {
            ...
            reject({
                ...
                long: `You cannot upload files of this type (${file.type}). Please try re-uploading a valid file type.`,
            });
            return;
        }
        ...
        if (!validateFileSize(file.size)) {
            ...
            reject({
                ...
                long: `The file you uploaded is too large. Please try re-uploading a smaller file size.`,
            });
            return;
        }
...
function handleSpecificTasks(data, setUploadData, setNotifications) {
    ...
    if (data.tasks.changePointsTask_id) {
        pollTaskStatus(data.tasks.changePointsTask_id, (result) => {
            ...
            handleTaskNotification(setNotifications, "Change Point analysis");
        }, (error) => {
            handleTaskError(error, setNotifications, "Change Point analysis");
        });
    }

    // Handle objectAnalysisTask
    if (data.tasks.objectAnalysisTask_id) {
        pollTaskStatus(data.tasks.objectAnalysisTask_id, (result) => {
            setUploadData(prevData => {
                // Clone the array to avoid direct mutation
                let newData = [...prevData];
            ...
            handleTaskNotification(setNotifications, "Object Analysis");
        }, (error) => {
            handleTaskError(error, setNotifications, "Text Analysis");
        });
    }

    // Handle privacyAssessmentTask
    if (data.tasks.privacyAssessmentTask_id) {
        pollTaskStatus(data.tasks.privacyAssessmentTask_id, (result) => {
            setUploadData(prevData => {
                // Clone the array to avoid direct mutation
                let newData = [...prevData];
            ...
            handleTaskNotification(setNotifications, "Privacy Assessment");
        }, (error) => {
            handleTaskError(error, setNotifications, "Privacy Assessment");
        });
    }

    // Handle distributionByNumericalTask
    if (data.tasks.distributionByNumericalTask_id) {
        pollTaskStatus(data.tasks.distributionByNumericalTask_id, (result) => {
            setUploadData(prevData => {
                // Clone the array to avoid direct mutation
                let newData = [...prevData];
            ...
            handleTaskNotification(setNotifications, "Categorical Distribution");
        }, (error) => {
            handleTaskError(error, setNotifications, "Categorical Distribution");
        });
    }
...
function updateUploadData(setUploadData, key, result) {
    setUploadData(prevData => {
        let newData = [...prevData];
        if (newData.length > 0) {
            let lastElement = { ...newData[newData.length - 1] };
            if (key === 'distribution_by_numerical') {
                mergeDistributionByNumerical(lastElement, result);
            } else {
                lastElement[key] = result;
            }
            newData[newData.length - 1] = lastElement;
        }
        return newData;
...
function mergeObjectAnalysis(lastElement, distributionResult) {
    const categoricalAnalysis = lastElement?.data?.statistical_summary?.categorical_analysis;
    if (categoricalAnalysis) {
        // Loop through each key in the distributionResult and merge
        Object.keys(distributionResult).forEach(columnKey => {
            if (categoricalAnalysis[columnKey]) {
                // Merge the distributionResult for this column into the existing categorical analysis
                categoricalAnalysis[columnKey] = {
                    ...categoricalAnalysis[columnKey],
                    ...distributionResult[columnKey]
                };
            } else {
                // If the key doesn't exist in categorical analysis, just add it
                categoricalAnalysis[columnKey] = distributionResult[columnKey];
            }
        });
    }
}
...
function handleTaskError(error, setNotifications, taskName) {
    setNotifications(prev => [...(prev || []), {
        type: "error",
        short: `Error during ${taskName}`,
        long: `There was an error processing the ${taskName} in the background. Please try re-uploading, and if the error persists, contact the developer by sending the feedback.`,
        time: getCurrentTime(),
        details: error,
    }]);
}
