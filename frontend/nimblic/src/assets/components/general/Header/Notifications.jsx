...
const Notifications = ({ notifications }) => {
  ...
  const handleSubmit = (feedback) => async (e) => {
    e.preventDefault();
      ...
      <div className="w-full h-fit max-h-96 overflow-y-auto my-6">
        {notifications && notifications?.length !== 0 ?
          Object.values(notifications).map((value, index) => (
            <div key={index} className={`collapse ${value.long ? 'collapse-arrow' : ''} bg-base-200 my-2`}>
              <input type="radio" name="my-accordion-2" />
              <div key={index} className={`collapse-title text-md font-semibold flex flex-row gap-2 justify-between items-center`}>
              ...
              {value?.long ?
                <div key={index} className="collapse-content flex flex-col items-center justify-center">
                  <div className="divider divider-neutral w-full mt-[-6px] mb-0"></div>
                  <p key={index}>{value.long}</p>
                  {value.type == 'warning' || value.type == 'error' &&
                    <form className="flex flex-row w-full items-end justify-end mt-6" onSubmit={() => handleSubmit(value)}>
                      <button
                        className={`btn w-40 ${sendSuccess ? "btn-secondary" : sendError ? "btn-error" : "btn-info"} text-md`}
                        type="submit">
                        {!isLoading ? (
                          sendSuccess ? (
                            <>
